...
namespace GladNet.Server.Common.Tests
{
	...
	public static class ServerPeerSessionTests
	{
		...
		{
			//assert
			Assert.DoesNotThrow(() => { var r = new Mock<ServerPeerSession>(Mock.Of<ILog>(), Mock.Of<INetworkMessageSender>(), Mock.Of<IConnectionDetails>(), Mock.Of<INetworkMessageSubscriptionService>(), Mock.Of<IDisconnectionServiceHandler>()).Object; });
		}
		...
		public static void Test_Registered_RequestMessage_With_NetMessageSubService()
		{
			...
			Mock<ServerPeerSession> peer = new Mock<ServerPeerSession>(Mock.Of<ILog>(), Mock.Of<INetworkMessageSender>(), Mock.Of<IConnectionDetails>(), subService.Object, Mock.Of<IDisconnectionServiceHandler>());
			peer.CallBase = true;
