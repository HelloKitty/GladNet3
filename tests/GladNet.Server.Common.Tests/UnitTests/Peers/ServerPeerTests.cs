...
namespace GladNet.Server.Common.Tests
{
	...
	public static class ServerPeerTests
	{
		...
		public static void Test_CanSend_IsRequest(OperationType opType, bool expectedResult)
		{
			...
			Mock<ServerPeer> peer = new Mock<ServerPeer>(Mock.Of<ILogger>(), sender.Object, Mock.Of<IConnectionDetails>(), Mock.Of<INetworkMessageSubscriptionService>(), Mock.Of<IDisconnectionServiceHandler>());
			peer.CallBase = true;
		...
		{
			//Assert
			Assert.DoesNotThrow(() => { var r = new Mock<ServerPeer>(Mock.Of<ILogger>(), Mock.Of<INetworkMessageSender>(), Mock.Of<IConnectionDetails>(), Mock.Of<INetworkMessageSubscriptionService>(), Mock.Of<IDisconnectionServiceHandler>()).Object; } );
		}
		...
		public static void Test_Registered_EventMessage_With_NetMessageSubService()
		{
			...
			Mock<ServerPeer> peer = new Mock<ServerPeer>(Mock.Of<ILogger>(), Mock.Of<INetworkMessageSender>(), Mock.Of<IConnectionDetails>(), subService.Object, Mock.Of<IDisconnectionServiceHandler>());
			peer.CallBase = true;
