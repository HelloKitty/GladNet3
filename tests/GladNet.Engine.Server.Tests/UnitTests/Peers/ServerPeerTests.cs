...
namespace GladNet.Server.Common.Tests
{
	...
	public static class ServerPeerTests
	{
		...
		{
			//Assert
			Assert.DoesNotThrow(() => { var r = new Mock<ClientPeer>(Mock.Of<ILog>(), Mock.Of<INetworkMessagePayloadSenderService>(), Mock.Of<IConnectionDetails>(), Mock.Of<INetworkMessageSubscriptionService>(), Mock.Of<IDisconnectionServiceHandler>()).Object; } );
		}
		...
		public static void Test_Registered_EventMessage_With_NetMessageSubService()
		{
			...
			Mock<ClientPeer> peer = new Mock<ClientPeer>(Mock.Of<ILog>(), Mock.Of<INetworkMessagePayloadSenderService>(), Mock.Of<IConnectionDetails>(), subService.Object, Mock.Of<IDisconnectionServiceHandler>());
			peer.CallBase = true;
